...
public class MidiFile {
    ...
    public ArrayList<Integer> tempovalues;
    public ArrayList<Integer> tempoticks;
    ...
    /** Get the Quarter Note of the song */
    public int getQuarternote() { return quarternote; }
    ...
    private void parse(byte[] rawdata) {
        ...
        tempovalues = new ArrayList<Integer>();
        tempoticks = new ArrayList<Integer>();
        ...
        for (ArrayList<MidiEvent> list : allevents) {
            for (MidiEvent mevent : list) {
                if (mevent.Metaevent == MetaEventTempo) {
                    ...
                    tempovalues.add(mevent.Tempo);
                    tempoticks.add(mevent.StartTime);
                }
    ...
    private ArrayList<MidiEvent> ReadTrack(MidiFileReader file) {
        ...
        int tracklen = file.ReadInt();  //event_num= track len
        ...
        while (file.GetOffset() < trackend) {
            ...
            if (eventflag >= EventNoteOn && eventflag < EventNoteOn + 16) {
                ...
                if (mevent.Metaevent == MetaEventTimeSignature) {
                    ...
                    for (int ii = 0; ii < mevent.Metalength; ii++) {
                        mevent.Tempo |= ((mevent.Value[ii] & 0xFF) << (8 * (mevent.Metalength - ii - 1)));
                    }
                }
                else if (mevent.Metaevent == MetaEventSequenceName) {
                    mevent.TrackName = new String(mevent.Value);
                }
