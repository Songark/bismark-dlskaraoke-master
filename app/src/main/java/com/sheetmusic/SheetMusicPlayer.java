 ...
 */
public class SheetMusicPlayer extends LinearLayout {
     ...
     */
    public SheetMusicPlayer(Activity activity) {
        super(activity);
    ...
    public static Point getPreferredSize(int screenwidth, int screenheight) {
        int height = (int) (5.0 * screenwidth / ( 2 + 42));
        height = height * 2/3 ;
    ...
    private void PlaySound(String filename) {
        ...
        try {
            ...
            input.close();
            player.prepare();
            player.start();  //Android Media Player
        }
        catch (IOException e) {
    ...
    Runnable DoPlay = new Runnable() {
      public void run() {
        ...
        CreateMidiFile();  //create temp Midi file with the calculated tempo.
        playstate = playing;

        //PlaySound(tempSoundFile);  //play midi file by using android Media Player
